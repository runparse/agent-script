...
import { tsCodeAgentPrompt } from './tsCodeAgent.prompt';

export interface ITsCodeAgent<TOutSchema extends TSchema>
  extends IMultiStepAgent<TOutSchema> {
  authorizedImports?: string[];
  sandbox?: vm.Context;
}

export interface ITsCodeAgentProps<TOutSchema extends TSchema>
  extends PartialBy<IMultiStepAgentProps<TOutSchema>, 'prompts'> {
  authorizedImports?: string[];
  sandbox?: vm.Context;
}

export class TsCodeAgent<TOutSchema extends TSchema>
  extends MultiStepAgent<TOutSchema>
  implements ITsCodeAgent<TOutSchema>
{
  authorizedImports: string[];
  sandbox: vm.Context;

  constructor(props: ITsCodeAgentProps<TOutSchema>) {
    const prompts = props.prompts || tsCodeAgentPrompt;
    super({ ...props, prompts });
  ...
  override async step(
    ...
    try {
      ...
      try {
        ...
        if (output) {
          memoryStep.observations = `-- Function call results --\n${truncateContent(
            output,
          )}`;
          this.logger.logMarkdown({
            content: output,
  ...
  async executeScript(
    ...
    const scriptToolCallResults: {
      result: unknown;
      name: string;
      variableName: string;
    }[] = [];
    const functionCallVariableNames =
      this.getFunctionCallVariableNames(scriptCode);
    ...
    this.tools.forEach((tool) => {
      this.sandbox[tool.name] = async (input: unknown) => {
        try {
          ...
          const printName = functionCallVariableNames[tool.name] || tool.name;
          scriptToolCallResults.push({
            result,
            name: tool.name,
  ...
  getFunctionCallVariableNames(scriptCode: string): Record<string, string> {
    const regex = /(?<variableName>\w+)\s*=\s*await\s+(?<functionName>\w+)\(/g;
    const match = scriptCode.matchAll(regex);

    if (!match) {
      return {};
    }

    return Array.from(match).reduce((acc, curr) => {
      acc[curr.groups!.functionName!] = curr.groups!.variableName!;
      return acc;
