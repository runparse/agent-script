...
export const codeAgentPrompt = {
  ...
  ${tools
    .map(
      ...
      Takes inputs: ${tool.inputSchema}
      Returns an output of type: ${tool.outputSchema}`,
    )
    .join('\n')}

  ${
    managed_agents && Object.values(managed_agents).length
      ? `
  ...
  planning: {
    ...
    update_plan_pre_messages: ({ task }: { task: string }) => ` 
    You are a world expert at making efficient plans to solve any task using a set of carefully crafted tools.
    ...
    managed_agent: {
      task: ({ name, task }: { name: string; task: string }) => ` 
    ...
    An agent tried to answer a user query but it got stuck and failed to do so. You are tasked with providing an answer instead. Here is the agent's memory:,`,
        post_messages: ({ task }: { task: string }) => ` 
